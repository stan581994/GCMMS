...
import type { Member, Household, AppUser, MemberStatus, UserRole, Calling, ClerkTask, ChildRecord, ChildRecordTask } from '@/types'
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    function fetchAll() {
      ...
      Promise.all([fetchMembers, fetchUsers, fetchCallings, fetchClerkTasks, fetchChildRecords, fetchChildRecordTasks]).finally(() => setLoading(false))
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      ...
      if (event === 'SIGNED_OUT') { setMembers(mockMembers); setUsers([]); setCallings([]); setClerkTasks([]); setChildRecords([]); setChildRecordTasks([]) }
    })
  ...
  return (
    <DataContext.Provider
      value={{
        members, households, users, callings, clerkTasks, childRecords, childRecordTasks, loading,
